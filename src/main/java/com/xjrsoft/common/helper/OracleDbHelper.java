...
public class OracleDbHelper extends DbHelper {
     ...
     * @title 构造函数
     */
    public OracleDbHelper(DbType dbType, String conn, String userName, String passWord, String dbName) throws SQLException, ClassNotFoundException {
        super(dbType, conn, userName, passWord,dbName);
    }
    public OracleDbHelper(DbType dbType, String dbLinkId) throws SQLException, ClassNotFoundException {
...
//        connection.close();
    ...
    public List<Map<String, Object>> getDataByIdAndTableName(String tableName) throws SQLException {
        ...
        String sql = wrapPageSql(new StringBuilder("SELECT * FROM ").append(tableName), new Page(1, 100)).toString();
...
//        super.getStatement().close();
    ...
    public String getCheckTableNamesSql(Integer nameNum) {
        StringBuilder sql = new StringBuilder("select TABLE_NAME as \"tableName\" from user_tables WHERE TABLE_NAME IN(");
    ...
    protected StringBuilder wrapPageSql(StringBuilder find, IPage page) {
        ...
        return find.insert(0, "SELECT * FROM ( SELECT row_.*, rownum rownum_ from ( ").append(" ) row_ where rownum <= ").append(startEnd[1]).append(") table_alias").append(" where table_alias.rownum_ > ").append(startEnd[0]);
    }
}